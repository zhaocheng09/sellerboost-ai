...
function InventoryPage() {
  ...
  function add() {
    if (!product.trim()) { toast.error("Pick or type a product"); return; }
  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground mt-1">Simple stock tracking — no spreadsheets needed.</p>
      </header>

      {/* Summary */}
      <div className="grid grid-cols-3 gap-3">
        <Card className="p-4">
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Products</div>
          <div className="text-2xl font-bold mt-1">{new Set(entries.map((e) => e.product)).size}</div>
        </Card>
        <Card className="p-4">
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Sold today</div>
          <div className="text-2xl font-bold mt-1">{todaySold}</div>
        </Card>
        <Card className={`p-4 ${lowStock.length > 0 ? "border-warning bg-warning/5" : ""}`}>
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Low stock</div>
      ...
      <Card className="p-5 space-y-4">
        <div className="font-semibold flex items-center gap-2"><Plus className="h-4 w-4" /> Add stock entry</div>
        <div>
          <Label htmlFor="prod">Product</Label>
          {savedProducts.length > 0 ? (
            <Select value={product} onValueChange={setProduct}>
              <SelectTrigger className="mt-1.5"><SelectValue placeholder="Pick saved or type custom" /></SelectTrigger>
              <SelectContent>
                {savedProducts.map((p) => <SelectItem key={p.id} value={p.name}>{p.name}</SelectItem>)}
              </SelectContent>
            </Select>
          ) : null}
          <Input id="prod" value={product} onChange={(e) => setProduct(e.target.value)} placeholder="Or type a product name" className="mt-2" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label htmlFor="st">Stock added</Label>
          ...
          <div>
            <Label htmlFor="sd">Sold today</Label>
        ...
        <div>
          <Label htmlFor="nt">Notes <span className="text-muted-foreground">(optional)</span></Label>
      ...
      {weekSold > 0 && (
        <Card className="p-5 bg-teal-soft border-secondary/20">
          <div className="text-xs uppercase tracking-wider font-semibold text-secondary">This week</div>
          <div className="text-base font-semibold mt-1">You sold {weekSold} units across {weekProducts} {weekProducts === 1 ? "product" : "products"} 🎉</div>
        </Card>
      )}

      {/* Recent entries */}
      <div>
        <h2 className="text-xs uppercase tracking-wider text-muted-foreground font-semibold mb-3">Recent entries</h2>
        {entries.length === 0 ? (
